    ...
    this.buildFunctionPattern = function() {
        ...
        for (var n = this.alignmentPatternCenters.length, i = 0; n > i; i++)
            for (var r = this.alignmentPatternCenters[i] - 2, o = 0; n > o; o++)
                0 == i && (0 == o || o == n - 1) || i == n - 1 && 0 == o || t.setRegion(this.alignmentPatternCenters[o] - 2, r, 5, 5);
        return t.setRegion(6, 9, 1, e - 17),
        t.setRegion(9, 6, e - 17, 1),
        6 < this.versionNumber && (t.setRegion(e - 11, 0, 3, 6),
...
function Detector(e) {
    ...
    this.sizeOfBlackWhiteBlackRun = function(e, t, n, i) {
        ...
        for (var s = Math.abs(n - e), a = Math.abs(i - t), h = -s >> 1, c = i > t ? 1 : -1, l = n > e ? 1 : -1, f = 0, d = e, o = t; d != n; d += l) {
            var w = r ? o : d
              , u = r ? d : o;
            if (1 == f ? this.image[w + u * qrcode.width] && f++ : this.image[w + u * qrcode.width] || f++,
            3 == f)
                return i = d - e,
                t = o - t,
                Math.sqrt(i * i + t * t);
    ...
    this.sizeOfBlackWhiteBlackRunBothWays = function(e, t, n, i) {
        ...
        return n = e - (n - e),
        0 > n ? (o = e / (e - n),
        n = 0) : n >= qrcode.width && (o = (qrcode.width - 1 - e) / (n - e),
        n = qrcode.width - 1),
        i = Math.floor(t - (i - t) * o),
        o = 1,
        0 > i ? (o = t / (t - i),
        i = 0) : i >= qrcode.height && (o = (qrcode.height - 1 - t) / (i - t),
        i = qrcode.height - 1),
    ...
    this.computeDimension = function(e, t, n, i) {
        switch (t = Math.round(this.distance(e, t) / i),
    ...
    this.findAlignmentInRegion = function(e, t, n, i) {
        ...
        return n = Math.min(qrcode.height - 1, n + r),
        new AlignmentPatternFinder(this.image,i,o,t - i,n - o,e,this.resultPointCallback).find()
    }
    ,
    this.createTransform = function(e, t, n, i, r) {
        ...
        return null != i ? (o = i.X,
        i = i.Y,
        s = a = r - 3) : (o = t.X - e.X + n.X,
        i = t.Y - e.Y + n.Y,
        s = a = r),
        PerspectiveTransform.quadrilateralToQuadrilateral(3.5, 3.5, r, 3.5, s, a, 3.5, r, e.X, e.Y, t.X, t.Y, o, i, n.X, n.Y)
    }
    ,
    ...
    this.processFinderPatternInfo = function(e) {
        ...
        return i = this.createTransform(t, n, e, a, r),
...
function BitMatrix(e, t) {
    ...
    ,
    this.clear = function() {
        for (var e = this.bits.length, t = 0; e > t; t++)
            this.bits[t] = 0
    }
    ,
...
function BitMatrixParser(e) {
    ...
    this.readFormatInformation = function() {
        ...
        for (e = this.copyBit(7, 8, e),
        e = this.copyBit(8, 8, e),
        e = this.copyBit(8, 7, e),
        t = 5; t >= 0; t--)
            e = this.copyBit(8, t, e);
        if (this.parsedFormatInfo = FormatInformation.decodeFormatInformation(e),
    ...
    this.readVersion = function() {
        ...
        var e = this.bitMatrix.Dimension
          , t = e - 17 >> 2;
        if (6 >= t)
            return Version.getVersionForNumber(t);
        for (var t = 0, n = e - 11, i = 5; i >= 0; i--)
            for (var r = e - 9; r >= n; r--)
                t = this.copyBit(r, i, t);
        if (this.parsedVersion = Version.decodeVersionInformation(t),
...
function ReedSolomonDecoder(e) {
    ...
    this.decode = function(e, t) {
        for (var n = new GF256Poly(this.field,e), i = Array(t), r = 0; r < i.length; r++)
            i[r] = 0;
        ...
        if (!o)
            for (r = new GF256Poly(this.field,i),
            ...
            r = 0; r < n.length; r++) {
                if (o = e.length - 1 - this.field.log(n[r]),
                0 > o)
    ...
    this.runEuclideanAlgorithm = function(e, t, n) {
        ...
        for (var i = this.field.One, r = this.field.Zero, o = this.field.Zero, s = this.field.One; t.Degree >= Math.floor(n / 2); ) {
            ...
            r = this.field.inverse(r); t.Degree >= e.Degree && !t.Zero; ) {
        ...
        if (n = s.getCoefficient(0),
        0 == n)
            throw "ReedSolomonException sigmaTilde(0) was zero";
        return n = this.field.inverse(n),
        e = s.multiply2(n),
        n = t.multiply2(n),
        [e, n]
    }
    ...
    this.findErrorLocations = function(e) {
        var t = e.Degree;
        if (1 == t)
            return Array(e.getCoefficient(1));
        for (var n = Array(t), i = 0, r = 1; 256 > r && t > i; r++)
            0 == e.evaluateAt(r) && (n[i] = this.field.inverse(r),
            i++);
        if (i != t)
    ...
    this.findErrorMagnitudes = function(e, t, n) {
        for (var i = t.length, r = Array(i), o = 0; i > o; o++) {
            for (var s = this.field.inverse(t[o]), a = 1, h = 0; i > h; h++)
                o != h && (a = this.field.multiply(a, GF256.addOrSubtract(1, this.field.multiply(t[h], s))));
...
function GF256Poly(e, t) {
    ...
    var n = t.length;
    if (n > 1 && 0 == t[0]) {
        for (var i = 1; n > i && 0 == t[i]; )
            i++;
        if (i == n)
            this.coefficients = e.Zero.coefficients;
        else {
            for (this.coefficients = Array(n - i),
            n = 0; n < this.coefficients.length; n++)
    ...
    this.addOrSubtract = function(t) {
        ...
        for (o = r; o < t.length; o++)
            i[o] = GF256.addOrSubtract(n[o - r], t[o]);
    ...
    this.multiplyByMonomial = function(e, t) {
        ...
        if (0 == t)
            return this.field.Zero;
        for (var n = this.coefficients.length, i = Array(n + e), r = 0; r < i.length; r++)
            i[r] = 0;
        for (r = 0; n > r; r++)
    ...
    this.divide = function(e) {
        ...
        if (e.Zero)
            throw "Divide by 0";
        for (var t = this.field.Zero, n = this, i = e.getCoefficient(e.Degree), i = this.field.inverse(i); n.Degree >= e.Degree && !n.Zero; )
...
function GF256(e) {
    ...
    this.buildMonomial = function(e, t) {
        ...
        if (0 == t)
            return zero;
        for (var n = Array(e + 1), i = 0; i < n.length; i++)
            n[i] = 0;
        return n[0] = t,
        new GF256Poly(this,n)
    }
    ,
    this.exp = function(e) {
...
function FinderPattern(e, t, n) {
    ...
    this.aboutEquals = function(e, t, n) {
        return Math.abs(t - this.y) <= e && Math.abs(n - this.x) <= e ? (e = Math.abs(e - this.estimatedModuleSize),
        1 >= e || 1 >= e / this.estimatedModuleSize) : !1
    }
}
...
function FinderPatternFinder() {
    ...
    this.foundPatternCross = function(e) {
        ...
        return 7 > t ? !1 : (t = Math.floor((t << INTEGER_MATH_SHIFT) / 7),
        n = Math.floor(t / 2),
        Math.abs(t - (e[0] << INTEGER_MATH_SHIFT)) < n && Math.abs(t - (e[1] << INTEGER_MATH_SHIFT)) < n && Math.abs(3 * t - (e[2] << INTEGER_MATH_SHIFT)) < 3 * n && Math.abs(t - (e[3] << INTEGER_MATH_SHIFT)) < n && Math.abs(t - (e[4] << INTEGER_MATH_SHIFT)) < n)
    }
    ,
    ...
    this.crossCheckVertical = function(e, t, n, i) {
        for (var r = this.image, o = qrcode.height, s = this.CrossCheckStateCount, a = e; a >= 0 && r[t + a * qrcode.width]; )
            s[2]++,
            a--;
        if (0 > a)
            return 0 / 0;
        for (; a >= 0 && !r[t + a * qrcode.width] && s[1] <= n; )
            s[1]++,
            a--;
        if (0 > a || s[1] > n)
            return 0 / 0;
        for (; a >= 0 && r[t + a * qrcode.width] && s[0] <= n; )
            s[0]++,
            a--;
        if (s[0] > n)
            return 0 / 0;
        for (a = e + 1; o > a && r[t + a * qrcode.width]; )
            s[2]++,
            a++;
        if (a == o)
            return 0 / 0;
        for (; o > a && !r[t + a * qrcode.width] && s[3] < n; )
            s[3]++,
            a++;
        if (a == o || s[3] >= n)
            return 0 / 0;
        for (; o > a && r[t + a * qrcode.width] && s[4] < n; )
            s[4]++,
            a++;
    ...
    this.crossCheckHorizontal = function(e, t, n, i) {
        for (var r = this.image, o = qrcode.width, s = this.CrossCheckStateCount, a = e; a >= 0 && r[a + t * qrcode.width]; )
            s[2]++,
            a--;
        if (0 > a)
            return 0 / 0;
        for (; a >= 0 && !r[a + t * qrcode.width] && s[1] <= n; )
            s[1]++,
            a--;
        if (0 > a || s[1] > n)
            return 0 / 0;
        for (; a >= 0 && r[a + t * qrcode.width] && s[0] <= n; )
            s[0]++,
            a--;
        if (s[0] > n)
            return 0 / 0;
        for (a = e + 1; o > a && r[a + t * qrcode.width]; )
            s[2]++,
            a++;
        if (a == o)
            return 0 / 0;
        for (; o > a && !r[a + t * qrcode.width] && s[3] < n; )
            s[3]++,
            a++;
        if (a == o || s[3] >= n)
            return 0 / 0;
        for (; o > a && r[a + t * qrcode.width] && s[4] < n; )
            s[4]++,
            a++;
    ...
    this.handlePossibleCenter = function(e, t, n) {
        ...
        if (n = this.centerFromEnd(e, n),
        t = this.crossCheckVertical(t, Math.floor(n), e[2], i),
        !isNaN(t) && (n = this.crossCheckHorizontal(Math.floor(n), Math.floor(t), e[2], i),
        !isNaN(n))) {
            e = i / 7;
            ...
            return i || (n = new FinderPattern(n,t,e),
    ...
    this.selectBestPatterns = function() {
        ...
        if (e > 3) {
            ...
            for (e = t / e,
            n = 0; n < this.possibleCenters.length && 3 < this.possibleCenters.length; n++)
                Math.abs(this.possibleCenters[n].EstimatedModuleSize - e) > .2 * e && (this.possibleCenters.remove(n),
                n--)
        }
    ...
    this.haveMultiplyConfirmedCenters = function() {
        for (var e = 0, t = 0, n = this.possibleCenters.length, i = 0; n > i; i++) {
            ...
            r.Count >= CENTER_QUORUM && (e++,
            t += r.EstimatedModuleSize)
        }
        if (3 > e)
            return !1;
        ...
        for (var r = !1, o = Array(5), s = i - 1; t > s && !r; s += i) {
            ...
            for (var a = o[4] = 0, h = 0; n > h; h++)
                if (e[h + s * qrcode.width])
                    ...
                    if (4 == a)
                        if (this.foundPatternCross(o)) {
                            if (a = this.handlePossibleCenter(o, s, h))
                                i = 2,
                                this.hasSkipped ? r = this.haveMultiplyConfirmedCenters() : (a = this.findRowSkip(),
                                a > o[2] && (s += a - o[2] - i,
                                h = n - 1));
                            else {
                                do
                                    h++;
                                while (n > h && !e[h + s * qrcode.width]);h--
                            }
                            a = 0,
                            o[0] = 0,
                            o[1] = 0,
                    ...
                    o[a]++;
            this.foundPatternCross(o) && this.handlePossibleCenter(o, s, n) && (i = o[0],
            this.hasSkipped && (r = haveMultiplyConfirmedCenters()))
        }
        return e = this.selectBestPatterns(),
...
function AlignmentPatternFinder(e, t, n, i, r, o, s) {
    ...
    this.crossCheckVertical = function(e, t, n, i) {
        ...
        if (a == o || s[1] > n)
            return 0 / 0;
        for (; o > a && !r[t + a * qrcode.width] && s[2] <= n; )
            s[2]++,
            a++;
    ...
    this.find = function() {
        for (var t = this.startX, r = this.height, o = t + i, s = n + (r >> 1), a = [0, 0, 0], h = 0; r > h; h++) {
            ...
            a[1] = 0,
            a[2] = 0;
            for (var l = t; o > l && !e[l + qrcode.width * c]; )
                l++;
            for (var f = 0; o > l; ) {
                if (e[l + c * qrcode.width])
                    if (1 == f)
                        ...
                        if (this.foundPatternCross(a) && (f = this.handlePossibleCenter(a, c, l),
                        null != f))
                            return f;
                        a[0] = a[2],
                        a[1] = 1,
            ...
            if (this.foundPatternCross(a) && (f = this.handlePossibleCenter(a, c, o),
            null != f))
                return f
        }
...
function QRCodeDataBlockReader(e, t, n) {
    ...
    9 >= t ? this.dataLengthMode = 0 : t >= 10 && 26 >= t ? this.dataLengthMode = 1 : t >= 27 && 40 >= t && (this.dataLengthMode = 2),
    this.getNextBits = function(e) {
        var t = 0;
        if (e < this.bitPointer + 1) {
            for (var n = 0, t = 0; e > t; t++)
                n += 1 << t;
        ...
        if (e < this.bitPointer + 1 + 8) {
            for (var i = 0, t = 0; t < this.bitPointer + 1; t++)
                i += 1 << t;
        ...
        if (e < this.bitPointer + 1 + 16) {
            for (t = n = i = 0; t < this.bitPointer + 1; t++)
                i += 1 << t;
            i = (this.blocks[this.blockPointer] & i) << e - (this.bitPointer + 1),
            this.blockPointer++;
            var r = this.blocks[this.blockPointer] << e - (this.bitPointer + 1 + 8);
            for (this.blockPointer++,
            t = 0; t < e - (this.bitPointer + 1 + 8); t++)
                n += 1 << t;
            return n <<= 8 - (e - (this.bitPointer + 1 + 8)),
            t = i + r + ((this.blocks[this.blockPointer] & n) >> 8 - (e - (this.bitPointer + 1 + 8))),
    ...
    this.getFigureString = function(e) {
        var t = 0
          , n = "";
        do
            e >= 3 ? (t = this.getNextBits(10),
            100 > t && (n += "0"),
            10 > t && (n += "0"),
            e -= 3) : 2 == e ? (t = this.getNextBits(7),
            10 > t && (n += "0"),
            e -= 2) : 1 == e && (t = this.getNextBits(4),
            e -= 1),
            n += t;
        while (e > 0);return n
    }
    ...
    this.__defineGetter__("DataByte", function() {
        for (var e = []; ; ) {
            ...
            if (dataLength = this.getDataLength(t),
            1 > dataLength)
                throw "Invalid data length: " + dataLength;
            switch (t) {
            case 1:
                for (var t = this.getFigureString(dataLength), n = Array(t.length), i = 0; i < t.length; i++)
                    n[i] = t.charCodeAt(i);
                e.push(n);
                break;
            case 2:
                for (t = this.getRomanAndFigureString(dataLength),
                n = Array(t.length),
                i = 0; i < t.length; i++)
                    n[i] = t.charCodeAt(i);
                e.push(n);
                break;
            case 4:
                t = this.get8bitByteArray(dataLength),
                e.push(t);
                break;
            case 8:
                t = this.getKanjiString(dataLength),
                e.push(t)
            }
        }
        return e
...
GridSampler = {
    ...
    sampleGrid3: function(e, t, n) {
        for (var i = new BitMatrix(t), r = Array(t << 1), o = 0; t > o; o++) {
            for (var s = r.length, a = o + .5, h = 0; s > h; h += 2)
                r[h] = (h >> 1) + .5,
                r[h + 1] = a;
            n.transformPoints1(r),
            GridSampler.checkAndNudgePoints(e, r);
            try {
                for (h = 0; s > h; h += 2) {
    ...
    sampleGridx: function(e, t, n, i, r, o, s, a, h, c, l, f, d, w, u, C, E, B) {
        return n = PerspectiveTransform.quadrilateralToQuadrilateral(n, i, r, o, s, a, h, c, l, f, d, w, u, C, E, B),
...
PerspectiveTransform.quadrilateralToQuadrilateral = function(e, t, n, i, r, o, s, a, h, c, l, f, d, w, u, C) {
    return e = this.quadrilateralToSquare(e, t, n, i, r, o, s, a),
...
PerspectiveTransform.squareToQuadrilateral = function(e, t, n, i, r, o, s, a) {
    return dy2 = a - o,
    dy3 = t - i + o - a,
    0 == dy2 && 0 == dy3 ? new PerspectiveTransform(n - e,r - n,e,i - t,o - i,t,0,0,1) : (dx1 = n - r,
    dx2 = s - r,
    dx3 = e - n + r - s,
    dy1 = i - o,
    ...
    new PerspectiveTransform(n - e + a13 * n,s - e + a23 * s,e,i - t + a13 * i,a - t + a23 * a,t,a13,a23,1))
}
,
...
FormatInformation.numBitsDiffering = function(e, t) {
    return e ^= t,
    BITS_SET_IN_HALF_BYTE[15 & e] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 4)] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 8)] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 12)] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 16)] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 20)] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 24)] + BITS_SET_IN_HALF_BYTE[15 & URShift(e, 28)]
}
,
...
FormatInformation.doDecodeFormatInformation = function(e) {
    for (var t = 4294967295, n = 0, i = 0; i < FORMAT_INFO_DECODE_LOOKUP.length; i++) {
        var r = FORMAT_INFO_DECODE_LOOKUP[i]
          , o = r[0];
        if (o == e)
            return new FormatInformation(r[1]);
        o = this.numBitsDiffering(e, o),
        t > o && (n = r[1],
        t = o)
    }
...
DataBlock.getDataBlocks = function(e, t, n) {
    ...
    for (t = n[0].codewords.length,
    r = n.length - 1; r >= 0 && n[r].codewords.length != t; )
        r--;
    for (r++,
    i = t - i.ECCodewordsPerBlock,
    t = a = 0; i > t; t++)
        for (s = 0; o > s; s++)
            n[s].codewords[t] = e[a++];
    for (s = r; o > s; s++)
        n[s].codewords[i] = e[a++];
    for (h = n[0].codewords.length,
    t = i; h > t; t++)
        for (s = 0; o > s; s++)
            n[s].codewords[r > s ? t : t + 1] = e[a++];
    return n
}
,
DataMask = {
...
Decoder.correctErrors = function(e, t) {
    for (var n = e.length, i = Array(n), r = 0; n > r; r++)
        i[r] = 255 & e[r];
    n = e.length - t;
    try {
...
Decoder.decode = function(e) {
    ...
    e = t.readVersion();
    for (var n = t.readFormatInformation().ErrorCorrectionLevel, t = t.readCodewords(), t = DataBlock.getDataBlocks(t, e, n), i = 0, r = 0; r < t.Length; r++)
        i += t[r].NumDataCodewords;
    for (var i = Array(i), o = 0, s = 0; s < t.length; s++) {
        var r = t[s]
          , a = r.Codewords
          , h = r.NumDataCodewords;
        for (Decoder.correctErrors(a, h),
        r = 0; h > r; r++)
            i[o++] = a[r]
    }
...
qrcode = {
    ...
    getPixel: function(e, t) {
        ...
        if (qrcode.height < t)
            throw "point error";
        return point = 4 * e + 4 * t * qrcode.width,
    ...
    grayScaleToBitmap: function(e) {
        for (var t = qrcode.getMiddleBrightnessPerArea(e), n = t.length, i = Math.floor(qrcode.width / n), r = Math.floor(qrcode.height / n), o = Array(qrcode.height * qrcode.width), s = 0; n > s; s++)
            for (var a = 0; n > a; a++)
                for (var h = 0; r > h; h++)
                    for (var c = 0; i > c; c++)
                        o[i * a + c + (r * s + h) * qrcode.width] = e[i * a + c + (r * s + h) * qrcode.width] < t[a][s] ? !0 : !1;
        return o
    },
    grayscale: function() {
...
Array.prototype.remove = function(e, t) {
    ...
    return this.length = 0 > e ? this.length + e : e,
